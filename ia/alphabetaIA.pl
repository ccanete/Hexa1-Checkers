...
alphabetaIA(Player, BestX, BestY, BestXdest, BestYdest):-
	...
	findMinMax(Player, Player, PossibleMoves, [], 3, null, null, _, [BestX, BestY, BestXdest, BestYdest]),
	setState(board).
...
findMinMax(Player, _, [], _, _, Score, MoveToProcess, NewScore, MoveToProcess):- 
	not(continuePlaying),
	getWinner(Player),
	NewScore is 500!.
...
%% If ennemi close to a queen piece, should be eaten !
